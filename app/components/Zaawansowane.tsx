...
export default function Zaawansowane(){
   ...
   const exportPlaylist = async (e:any) => {
      ...
      if(!response){
    return(
        <div className='absolute top-10 z-0 h-5/6 p-4 w-5/6'>
   ...
   console.log(response);
    return(
        ...
        <div key={item.track_name} className=' h-16 w-full flex flex-col justify-center bg-senary rounded-md mb-1 mt-1' data-value={item.track_id} onClick={PassTrack}>
            <p className='font-bold'>{item.track_name}</p>
            <p>{item.artists}</p>
        </div>
         ))}
